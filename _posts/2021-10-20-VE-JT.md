...
* **Distributive property**: This describes how we can distribute the summing process used in variable elimination.
$$\phi(X) = \phi_1(X) \cdot \phi_2(X)$$
$$\sum_{X_s} \phi$$ and $$X_s \notin \text{scope}(\phi_1)$$
Then $$\sum_{X_s} \phi_1 \phi_2 = \phi_1 \sum_{X_s}\phi_2$$

* **Factor marginalization**: This describes how we can marginalize over factors with variable elimination.
$$\Psi(X - \{Y\}) = \sum_Y \phi(X)$$
