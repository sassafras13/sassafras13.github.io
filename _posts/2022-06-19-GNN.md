...
Note that the meaning of these factors varies slightly depending on which type of graphical model you are working with. For directed graphs, the factors are conditional probabilities with respect to the node’s parents, i.e. $$\{ P(X_i | X_{PA_i}) \}$$. For undirected graphs, the factors are called **clique potentials** and are written as $$\phi_c (X_c)$$. Clique potentials become probabilities after they are normalized [1]. If we wanted to compute the marginal probability of $$P(X_5)$$, we would sum over all of the other variables, like so [2]: 
...
Observe that the last term, $$\sum_{X_1}  P(X_1) P(X_2 | X_1)$$, can be simplified to $$P(X_2)$$, to give us [2]: 
